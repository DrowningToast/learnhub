            ...
            <div class="flex flex-col gap-y-2 w-full    ">
                ...
                <div class="flex flex-col gap-y-2 w-full">
                    <div class="w-full">
                        ...
                        <label for="a" class="rounded-xl px-5 py-3 peer-checked:bg-gradient-to-r peer-checked:from-[#2A638A] peer-checked:to-[#0B1A24] peer-checked:text-white text-xl w-full flex gap-x-3"><span class="font-bold">ก.</span>Lorem ipsum dolor sit amet consectetur.</label>
                    </div>
                    <div class="w-full">
                        <input type="radio" name="choice" value="b" id="b" class="hidden peer"/>
                        <label for="b" class="rounded-xl px-5 py-3 peer-checked:bg-gradient-to-r peer-checked:from-[#2A638A] peer-checked:to-[#0B1A24] peer-checked:text-white text-xl w-full flex gap-x-3"><span class="font-bold">ข.</span>Lorem ipsum dolor sit amet consectetur.</label>
                    </div>
                    <div class="w-full">
                        <input type="radio" name="choice" value="c" id="c" class="hidden peer"/>
                        <label for="c" class="rounded-xl px-5 py-3 peer-checked:bg-gradient-to-r peer-checked:from-[#2A638A] peer-checked:to-[#0B1A24] peer-checked:text-white text-xl w-full flex gap-x-3"><span class="font-bold">ค.</span>Lorem ipsum dolor sit amet consectetur.</label>
                    </div>
                </div>
                {{-- <div>
                    <p>กรุณาเลือกคำตอบ </p>
                ...
                <div class="flex justify-center gap-x-3 w-full font-noto-thai mt-3">
                    @if (app('request')->input('no') == 1)
                        <a href="{{ url()->current() }}" onclick="localStorage.removeItem('answers')" class="rounded-full text-[#2A638A] bg-[#E9F2FC] px-14 py-3">ยกเลิก</a>
                    @else
                        <a href="{{ url()->current() }}/?doquiz=true&no={{ (int)app('request')->input('no') - 1 }}" class="rounded-full text-[#2A638A] bg-[#E9F2FC] px-14 py-3">ก่อนหน้า</a>
                    @endif

                    @if (app('request')->input('no') == 3)
                        <button onclick="submit()" class="rounded-full bg-[#2A638A] text-white px-14 py-3">ส่งคำตอบ</button>
                    @else
                        <button onclick="nextQuestion({{ (int)app('request')->input('no') }})" class="rounded-full bg-[#2A638A] text-white px-14 py-3">ถัดไป</button>
                    @endif
                </div>
            </div>
        </div>
