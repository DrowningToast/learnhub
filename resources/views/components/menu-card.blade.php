<div class="h-[290px] {{ $bgColor }} {{ $shColor }} hover:shadow-xl rounded-xl px-14 py-10 flex justify-center gap-x-24 duration-200">
    <img src="{{{ $imgSrc }}}" alt="manage course" class="h-full">
    <div class="flex flex-col justify-center gap-y-4">
        <h1 class="font-bold text-4xl text-ellipsis line-clamp-2">{{ $title }}</h1>
        <p class="text-[#A8ACAC] text-ellipsis line-clamp-3">{{ $description }}</p>
        @if ($author != "")
