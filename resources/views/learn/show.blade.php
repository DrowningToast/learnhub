<x-left_side_layout>
    ...
    <div class="mt-8 space-y-4">
        ...
        <form class="flex flex-col gap-y-4" action="/learn/{{$course->id}}/review" method="POST" >
            @csrf
            <div class="flex flex-col gap-y-4 border-[1px] border-[#DBE3EE] rounded-2xl p-6">
                <div class="flex">
                    ...
                    <div class="ml-auto flex items-center gap-x-1">
                        ...
                        <label onclick="setRating(1)" for="rating-1">
                            <img class="w-6 h-6" id="star-1" src="{{asset("/assets/star-1.png")}}" alt="score-1">
                        </label>
                        <input 
                            {{ (old("rating") == '1' || $review->rating == 1) ? 'checked' : '' }}
                            type="radio"
                            name="rating"
                            id="rating-1"
                            class="hidden"
                        ...
                        <label onclick="setRating(2)" for="rating-2">
                            <img class="w-6 h-6" id="star-2" src="{{asset("/assets/star-1.png")}}" alt="score-2">
                        </label>
                        <input 
                            {{ (old("rating") == '2' || $review->rating == 2) ? 'checked' : '' }}
                            type="radio"
                            name="rating"
                            id="rating-2"
                            class="hidden"
                        ...
                        <label onclick="setRating(3)" for="rating-3">
                            <img class="w-6 h-6" id="star-3" src="{{asset("/assets/star-1.png")}}" alt="score-3">
                        </label>
                        <input 
                            {{ (old("rating") == '3' || $review->rating == 3) ? 'checked' : '' }}
                            type="radio"
                            name="rating"
                            id="rating-3"
                            class="hidden"
                        ...
                        <label onclick="setRating(4)" for="rating-4">
                            <img class="w-6 h-6" id="star-4" src="{{asset("/assets/star-1.png")}}" alt="score-4">
                        </label>
                        <input 
                            {{ (old("rating") == '4' || $review->rating == 4) ? 'checked' : '' }}
                            type="radio"
                            name="rating"
                            id="rating-4"
                            class="hidden"
                        ...
                        <label onclick="setRating(5)" for="rating-5">
                            <img class="w-6 h-6" id="star-5" src="{{asset("/assets/star-1.png")}}" alt="score-5">
                        </label>
                        <input 
                            {{ (old("rating") == '5' || $review->rating == 5) ? 'checked' : '' }}
                            type="radio"
                            name="rating"
                            id="rating-5"
                            class="hidden"
                ...
                @error('comment')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
                <textarea name="comment" id="" cols="30" rows="5" placeholder="เขียนความคิดเห็นสำหรับคอร์สเรียนนี้ของคุณ..." class="bg-white text-lg rounded-md"  >{{old('comment') ??$review->comment}}</textarea>
            </div>
            <div class="ml-auto">
